...
void test_basicRestingOrder(){
    ...
    auto book = std::make_unique<OrderBookSide>(900);
...
void test_priceOverflowRejected(){
    ...
    AddOrderState result_valid   = book->addOrder(1, 1000, 100, true);   // in range
    AddOrderState result_too_low = book->addOrder(2, 500,  100, true);   // below base — invalid
    AddOrderState result_too_high= book->addOrder(3, 5000, 100, true);   // way above range — invalid

    CHECK(result_valid   == AddOrderState::COMPLETED);
    CHECK(result_too_low == AddOrderState::REJECTED);
    CHECK(result_too_high == AddOrderState::REJECTED);
...
void test_aggressiveSellMatchesFIFO(){
    ...
    AddOrderState ok = book->addOrder(4, 990, 150, false);

    CHECK(ok == AddOrderState::COMPLETED);
    CHECK(fills.size() == 3);
    if(fills.size() == 3){
